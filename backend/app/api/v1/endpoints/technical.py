...
from app.models.schemas import AptSectionCreate, AptQuestionCreate, AptTestCreate
...
async def create_section(data: AptSectionCreate, mode: str = "practice", db=Depends(get_db), user=Depends(require_faculty)):
    doc = {**data.model_dump(), "type": "technical", "mode": mode, "is_active": data.is_active, "created_by": user["id"], "created_at": datetime.utcnow()}
...
async def list_sections(
    ...
    for s in sections:
        s["id"] = str(s.pop("_id"))
        s["question_count"] = await db.apt_questions.count_documents({"section_id": s["id"], "section_type": "technical"})
        test_query = {"section_id": s["id"], "section_type": "technical"}
        if mode:
...
async def delete_section(section_id: str, db=Depends(get_db), _=Depends(require_faculty)):
    ...
    await db.apt_questions.delete_many({"section_id": section_id, "section_type": "technical"})
    await db.tests.delete_many({"section_id": section_id, "section_type": "technical"})
...
async def create_question(data: AptQuestionCreate, db=Depends(get_db), user=Depends(require_faculty)):
    ...
    doc = {**payload, "section_type": "technical", "is_active": data.is_active, "created_by": user["id"], "created_at": datetime.utcnow()}
...
async def download_question_template(_=Depends(require_faculty)):
    return build_excel_template(QUESTION_TEMPLATE_COLUMNS, "technical_questions_template.xlsx", sheet_name="Questions")


@router.post("/tests", status_code=201)
async def create_test(data: AptTestCreate, db=Depends(get_db), user=Depends(require_faculty)):
    doc = {**data.model_dump(), "section_type": "technical", "is_active": data.is_active, "created_by": user["id"], "created_at": datetime.utcnow()}
...
async def update_test(test_id: str, data: AptTestCreate, db=Depends(get_db), _=Depends(require_faculty)):
    await db.tests.update_one(
        ...
        {"$set": {**data.model_dump(), "section_type": "technical"}},
    )
    return {"message": "Updated"}
...
async def toggle_test(test_id: str, db=Depends(get_db), _=Depends(require_faculty)):
    test = await db.tests.find_one({"_id": ObjectId(test_id), "section_type": "technical"})
    if not test:
        raise HTTPException(404, "Test not found")
