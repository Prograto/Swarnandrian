...
async def evaluation_overview(db=Depends(get_db), user=Depends(require_faculty)):
    ...
    for s in recent_subs[:8]:
        recent_activity.append({
            "type": "submission",
            "label": f"{s.get('student_name', 'Student')} submitted {s.get('problem_name', 'a problem')}",
            "status": s.get("status"),
...
async def evaluation_results(
    ...
    for row in code_rows:
        student = student_map.get(str(row.get("student_id")), {})
        student_display_name = student.get("name") or row.get("student_id") or "Unknown"
        student_reg_no = student.get("reg_no")
        student_branch = student.get("branch") or "Unknown"
        ...
        if branch and branch != "all" and student_branch != branch:
            continue
        if student or student_name or reg_no:
            combined_needle = (student or "").strip() or None
