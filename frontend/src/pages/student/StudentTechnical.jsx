...
export default function StudentTechnical() {
  ...
  const { data, isLoading } = useQuery(['tech-sections-student', mode, search, page], () =>
    api.get('/technical/sections', { params: { mode, search, page, limit: PAGE_SIZE, paginate: true } }).then(r => r.data)
  );
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6 space-y-5">
      <div>
        ...
        <p className="text-gray-400 text-sm mt-0.5">Browse technical concepts and open related test cards in a dedicated page.</p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {sections.map((section, index) => (
            ...
            >
              <div className="h-32 bg-gradient-to-br from-[#eafdf1] to-[#f4fff8]">
              ...
              <div className="p-4 space-y-3">
                ...
                <p className="text-xs text-secondary line-clamp-2">{section.description || 'OS, DBMS, CN, OOP, and system design test concepts.'}</p>
                <div className="flex items-center justify-between">
                  <span className="text-xs text-secondary">{section.question_count || 0} Questions</span>
                  <Link to={`/student/technical/${mode}/section/${section.id}`} className="btn-primary text-xs">Open Concept</Link>
                </div>
              </div>
            </motion.div>
          ))}
