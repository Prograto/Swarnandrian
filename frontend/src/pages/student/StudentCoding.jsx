...
const GRADIENTS = [
  'from-[#EEF2FF] to-[#E0E7FF]','from-[#ECFDF5] to-[#D1FAE5]',
  'from-[#EFF6FF] to-[#DBEAFE]','from-[#F5F3FF] to-[#EDE9FE]',
  'from-[#FDF2F8] to-[#FCE7F3]','from-[#FFFBEB] to-[#FEF3C7]',
];
...
export default function StudentCoding() {
  ...
  const { data, isLoading } = useQuery(
    ['coding-sections-student', mode, page, search, diff],
    () => api.get('/coding/sections', {
    ...
    <div className="max-w-7xl mx-auto px-4 py-6 space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="section-title flex items-center gap-2">
            {mode === 'practice'
              ? <><MenuBookRoundedIcon sx={{ fontSize: 22 }} className="text-[#4F7CF3]" /> Coding Practice</>
              : <><EmojiEventsRoundedIcon sx={{ fontSize: 22 }} className="text-amber-500" /> Coding Competitor</>}
          </h1>
          <p className="text-sm text-secondary mt-0.5">{mode === 'practice' ? 'Unlimited attempts · DSA problems' : 'Scored mode · Climb the leaderboard'}</p>
        </div>
        <Link to={mode === 'practice' ? '/student/coding/competitor' : '/student/coding/practice'}
      ...
      <div className="flex gap-3">
        <label className="flex items-center gap-2 flex-1 rounded-2xl border border-theme bg-surface-card px-3 py-2.5 focus-within:border-[#4F7CF3] focus-within:shadow-[0_0_0_3px_rgba(79,124,243,0.08)] transition-all">
          ...
          <input value={search} onChange={e => { setSearch(e.target.value); setPage(1); }}
            placeholder="Search coding concepts…" className="w-full bg-transparent text-sm text-primary outline-none placeholder:text-muted" />
        </label>
      ...
      {isLoading ? (
        ...
        <div className="card p-12 text-center">
          <CodeRoundedIcon sx={{ fontSize: 40 }} className="text-muted mx-auto mb-3" />
            ...
            <motion.div key={section.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.04 }}
              ...
              <div className={`h-28 bg-gradient-to-br ${GRADIENTS[index % GRADIENTS.length]} relative flex-shrink-0`}>
                {section.banner_url
                  ...
                  : <div className="h-full w-full flex items-center justify-center opacity-20"><CodeRoundedIcon sx={{ fontSize: 48 }} /></div>}
              ...
              <div className="p-4 flex flex-col flex-1 gap-3">
                <div>
                  ...
                  <p className="text-xs text-secondary mt-1 line-clamp-2">{section.description || 'Practice this concept with curated coding problems.'}</p>
                </div>
                <div className="flex items-center gap-2 mt-auto text-xs text-secondary">
                  <CodeRoundedIcon sx={{ fontSize: 14 }} />{section.problem_count || 0} Problem{(section.problem_count || 0) !== 1 ? 's' : ''}
                </div>
                <Link to={`/student/coding/${mode}/section/${section.id}`}
