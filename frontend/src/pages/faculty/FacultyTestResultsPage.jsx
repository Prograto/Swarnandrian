...
export default function FacultyTestResultsPage() {
  ...
  const { data: results, isLoading } = useQuery(
    ['faculty-test-results', testId, page, filters],
    () => api.get(`/faculty/evaluation/test/${testId}/results`, {
      params: {
        page,
        limit: 20,
  ...
  const exportResults = async (format) => {
    const res = await api.get(`/faculty/evaluation/test/${testId}/results/export`, {
      params: {
        start_date: filters.startDate || undefined,
        end_date: filters.endDate || undefined,
        student: filters.search || undefined,
    ...
    a.href = url;
    a.download = `test_results_${testId}.${format === 'csv' ? 'csv' : 'xlsx'}`;
    a.click();
      ...
      <div className="space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-secondary mt-1">Showing only results for this test: {results?.test?.name || testId}</p>
          </div>
            ...
            <table className="w-full">
              ...
              <tbody>
                {isLoading ? (
                  ...
                  <tr><td colSpan={8} className="tbl-td text-secondary">No results found for this test.</td></tr>
